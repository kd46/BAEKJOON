...
public class P11650 {
	...
	private static int[][] sortX(int[][] arr) {
		if(arr.length < 2) return arr;
		
		int mid = arr.length / 2;
		
		/* 0 ~ mid-1 */
		int[][] low = sortX(copyOfRange(arr, 0, mid));
		
		/* mid ~ end */
		int[][] high = sortX(copyOfRange(arr, mid, arr.length));
		...
		while(l < low.length && h < high.length) {
			...
			/* x가 같을 때 y끼리 비교해서 low가 작을 경우 */
			} else if(low[l][0] == high[h][0] 
					&& low[l][1] < high[h][1]) {
				merged[m][0] = low[l][0];
				merged[m++][1] = low[l++][1];
				
			/* x는 같지만 y는 high가 클경우, 둘다 high가 클경우 */
			} else {
				merged[m][0] = high[h][0];
				merged[m++][1] = high[h++][1];
			}
